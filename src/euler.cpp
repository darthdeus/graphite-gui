...
void Euler::step() {
  ...
  bool undeleted_count = std::count_if(current_->edges.begin(),
                                       current_->edges.end(), is_not_deleted);
  ...
  bool all_bridges = std::all_of(current_->edges.begin(), current_->edges.end(),
                                 is_bridge_or_deleted);
  if (all_bridges) {
    // If we got this far there's at least one edge
    assert(current_->edges.size() > 0);

    Edge& edge = *std::find_if(current_->edges.begin(), current_->edges.end(),
  ...
  auto res = std::find_if_not(current_->edges.begin(), current_->edges.end(),
                              is_bridge_or_deleted);
